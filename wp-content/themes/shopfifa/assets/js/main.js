...
(function($) {
    ...
    function openMenuBurger() {
        $('.btn-collapse').click(function(e) {
            if (!$('.btn-collapse').hasClass('is-active')) {
                $('html').addClass('open-menu');
                $('.btn-collapse').addClass('is-active');
    ...
    function openSubmenu() {
        $('.has-sub >a').click(function(e) {
            ...
            if (!$item.hasClass('is-active')) {
                ...
                $item.find('.sub-menu').addClass('is-active');
            } else {
                $item.removeClass('is-active');
                $item.find('.sub-menu').removeClass('is-active');
            }
        });
    }
    function changePage() {
        $('#click-page').click(function(e) {
            $(this).parent().addClass('not-show');
        });
    }
    function sliderList() {
        $('#list-slider1').slick({
            ...
            rows: 0,
            infinite: true,
            prevArrow: '<button class="slick-prev slick-arrow"><i class="icomoon icon-angle-left"></i></button>',
            nextArrow: '<button class="slick-next slick-arrow"><i class="icomoon icon-angle-right"></i></button>'
        });
    }
    ...
    function slideDetail() {
        $('.block-detail .content-slider').slick({
            infinite: true,
            slidesToShow: 1,
            variableWidth: true,
            centerMode: true,
    ...
    function fullInfoOrange() {
        $('.btn-full-info-orange').click(function(event) {
            event.preventDefault();
            $(this).parent().hide();
            $('.full-info-orange').show();
        });
    }
    function changeColorRating() {
        $('.rating li').on('click', function() {
            var onColor = parseInt($(this).data('value'), 10);
 // The color currently selected
                        var colors = $(this).parent().children('li.color-item');
            $('li.color-item').removeClass('active');
            $(this).addClass('active');
            for (var i = 0; i < colors.length; i++) {
                $(colors[i]).removeClass('selected');
            }
            for (var i = 0; i < onColor; i++) {
                $(colors[i]).addClass('selected');
            }
        });
    }
})(jQuery);
